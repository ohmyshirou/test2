...
@section('content')
    ...
    <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        ...
        <table class="min-w-full mt-4 table-auto border-collapse">
            <thead>
                <tr class="border-b-2 border-gray-300">
                    <th class="py-2 px-4 text-left text-sm font-semibold">Proposal Name</th>
                    <th class="py-2 px-4 text-left text-sm font-semibold">Status</th>
                    <th class="py-2 px-4 text-left text-sm font-semibold">Last Upload</th>
                </tr>
            </thead>
            <tbody>
                ...
                @else
                    @foreach ($recentProposals as $proposal)
                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                            ...
                            <td class="py-2 px-4">{{ $proposal->updated_at->format('Y-m-d') }}</td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
        ...
        <div class="mt-4 flex justify-center">
            {{ $recentProposals->links() }} <!-- Link pagination -->
        </div>
    </div>

    <!-- Kalender Organization -->
    <div class="bg-white p-6 rounded-lg shadow-md mt-6">
        <!-- Header Kalender -->
